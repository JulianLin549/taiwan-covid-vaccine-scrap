...
const url = "https://www03.taic.mohw.gov.tw/OINetReg/OINetReg.Reg/Reg_RegTable.aspx?HID=F&Way=Dept&DivDr=01CD&Date=&Noon="
...
const getData = async (browser) => {
    ...
    try {
        ...
        console.log("衛生福利部台中醫院:", data)
        await page.waitForTimeout(process.env.DELAY_TIME);
        await page.close();
        await saveToDb("衛生福利部台中醫院", data);
    } catch (e) {
        await page.close();
